...
namespace Utility.Editor
{
	...
	public class RuleTileEditor : UnityEditor.Editor
	{
		...
		private static void RuleMatrixOnGUI(Rect rect, RuleTile.TilingRule tilingRule)
		{
			...
			for (int y = 0; y <= 2; y++)
			{
				for (int x = 0; x <= 2; x++)
				{
					...
					{
						switch (tilingRule.mAutoTransform)
						{
						...
						if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
						{
							tilingRule.mAutoTransform = (RuleTile.TilingRule.AutoTransform)(((int)tilingRule.mAutoTransform + 1) % 4);
							GUI.changed = true;
							Event.current.Use();
						}
					}
		...
		private static void OnSelect(object userdata)
		{
			...
			data.m_Rule.mAutoTransform = data.m_NewValue;
		}

		private class MenuItemData
		{
			public RuleTile.TilingRule m_Rule;
			public RuleTile.TilingRule.AutoTransform m_NewValue;

			public MenuItemData(RuleTile.TilingRule mRule, RuleTile.TilingRule.AutoTransform mNewValue)
			{
				this.m_Rule = mRule;
		...
		{
			tilingRule.mSprites[0] = EditorGUI.ObjectField(new Rect(rect.xMax - rect.height, rect.yMin, rect.height, rect.height), tilingRule.mSprites[0], typeof (ChangingTile), false) as ChangingTile;
		}
		...
		private static void RuleInspectorOnGUI(Rect rect, RuleTile.TilingRule tilingRule)
		{
			...
			tilingRule.mAutoTransform = (RuleTile.TilingRule.AutoTransform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.mAutoTransform);
			...
			if (tilingRule.mOutput != RuleTile.TilingRule.OutputSprite.Single)
			{
				...
				EditorGUI.BeginChangeCheck();
				int newLength = EditorGUI.IntField(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.mSprites.Length);
				if (EditorGUI.EndChangeCheck())
				...
				{
					tilingRule.mSprites[i] = EditorGUI.ObjectField(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.mSprites[i], typeof(ChangingTile), false) as ChangingTile;
					y += k_SingleLineHeight;
				}
			}
		}
